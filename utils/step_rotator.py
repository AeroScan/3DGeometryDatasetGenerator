...
def rotateStepFile(step_path: str, axis: str, angle: float):
    print(f'Opening file...')
    shape = read_step_file(filename=step_path)
    print(f'Done.\n')
    if axis == 'x':
    ...
    print(f'Done.\n')
    makeNewStepFile(new_shape=shape, filename=step_path)

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Step Rotation')
    parser.add_argument('input_path', type=str, default='.', help='path to input directory or input files.')
    ...
    for file in files:
        ...
        rotateStepFile(step_path=file, axis=axis, angle=angle)
