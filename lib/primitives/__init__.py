...
def toDict(brep_adaptor, mesh_data=None, transforms=None):
    if isinstance(brep_adaptor, BRepAdaptor_Curve):
        ...
        if cls_name in CURVE_CLASSES:
            return CURVE_CLASSES[cls_name].toDict(brep_adaptor, mesh_data=mesh_data, transforms=transforms)
        else:
            return None
        ...
        if cls_name in SURFACE_CLASSES:
            return SURFACE_CLASSES[cls_name].toDict(brep_adaptor, mesh_data=mesh_data, transforms=transforms)
        else:
            return None
    else:
